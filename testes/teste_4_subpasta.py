...
with sync_playwright() as p:
    ...
    pg.wait_for_function("caches.open('reciclalito-v13').then(c=>c.keys()).then(k=>k.length>=10)", timeout=10000)
    n=pg.evaluate("caches.open('reciclalito-v13').then(c=>c.keys()).then(k=>k.length)")
